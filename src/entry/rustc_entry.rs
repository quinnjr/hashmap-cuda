...
unsafe impl<K, V> Send for RustcOccupiedEntry<'_, K, V> where K: Send, V: Send
{
  // Intentionally left blank
}

unsafe impl<K, V> Sync for RustcOccupiedEntry<'_, K, V> where K: Sync, V: Sync
{
  // Intentionally left blank
}
...
impl<'a, K, V> RustcOccupiedEntry<'a, K, V> {
  ...
  ///
  /// If you need a reference to the `RustcOccupiedEntry`
  ...
  /// Converts the RustcOccupiedEntry into a mutable reference
  /// to the value in the entry
  /// with a lifetime bound to the map itself.
  ///
  /// If you need multiple references to the `RustcOccupiedEntry`,
  /// see [`get_mut`].
  ///
...
impl<K, V, S> HashMap<K, V, S> where K: Eq + Hash, S: BuildHasher {
  ...
  pub fn rustc_entry(&mut self, key: K) -> RustcEntry<'_, K, V> {
    ...
    if let Some(elem) = self.table.find(hash, |q| q.0.eq(&key)) {
      ...
      // generic over the BuildHasher and adding a generic parameter

...
/// Primary hash function, used to select the initial bucket to
/// probe from.
#[inline]
...
#[allow(clippy::cast_possible_truncation)]
fn h2(hash: u64) -> u8 {
  // Grab the top 7 bits of the hash. While the hash is
  // normally a full 64-bit value, some hash functions
  // (such as FxHash) produce a usize result instead, which
  // means that the top 32 bits are 0 on 32-bit platforms.
...
/// (since our table size is a power of two) to visit every group
/// of elements exactly once.
///
/// A triangular probe has us jump by 1 more group every time. So
/// first we jump by 1 group (meaning we just continue our linear
/// scan), then 2 groups (skipping over 1 group), then 3
/// groups (skipping over 2 groups), and so on.
///
...
/// Returns the number of buckets needed to hold the given number
/// of items taking the maximum load factor into account.
///
...
fn capacity_to_buckets(cap: usize) -> Option<usize> {
  ...
  // Also, any rounding errors from the division above will be
  // cleaned up by next_power_of_two (which can't overflow because
  // of the previous divison).
  Some(adjusted_cap.next_power_of_two())
}

/// Returns the maximum effective capacity for the given bucket
/// mask, taking the maximum load factor into account.
#[inline]
fn bucket_mask_to_capacity(bucket_mask: usize) -> usize {
  if bucket_mask < 8 {
    // For tables with 1/2/4/8 buckets, we always reserve one
    // empty slot. Keep in mind that the bucket mask is one less
...
fn calculate_layout<T>(buckets: usize) -> Option<(Layout, usize)> {
  ...
  // Array of control bytes. This must be aligned to the
  // group size.
  //
  // We add `Group::WIDTH` control bytes at the end of the array
  // which replicate the bytes at the start of the array and thus
  // avoids the need to perform bounds-checking while probing.
  //
  // There is no possible overflow here since buckets is a
  // power of two and Group::WIDTH is a small number.
  let ctrl = unsafe {
...
// Returns a Layout which describes the allocation
// required for a hash table, and the offset of the buckets
// in the allocation.
...
fn calculate_layout<T>(buckets: usize) -> Option<(Layout, usize)> {
  ...
  // Manual layout calculation since Layout methods
  // are not yet stable.
  let data_align = usize::max(mem::align_of::<T>(), Group::WIDTH);
  let data_offset = (buckets + Group::WIDTH)
    .checked_add(data_align - 1)? & !(data_align - 1);
  let len = data_offset.checked_add(mem::size_of::<T>()
    .checked_mul(buckets)?
  )?;
...
/// However if the element is a ZST, then we instead track
/// the index of the element in the table so that `erase`
/// works properly.
pub struct Bucket<T> {
...
pub struct RawTable<T> {
  ...
  data: NonNull<T>,
  /// Number of elements that can be inserted before
...
impl<T> RawTable<T> {
  ...
  unsafe fn new_uninitialized(
    ...
    debug_assert!(buckets.is_power_of_two());
    let (layout, data_offset) = calculate_layout::<T>(buckets)
      .ok_or_else(|| fallability.capacity_overflow())?;
    let ctrl = NonNull::new(alloc(layout))
        .ok_or_else(|| fallability.alloc_err(layout))?;
  ...
  /// Attempts to allocate a new hash table with at least
  /// enough capacity for inserting the given number of elements
  ...
  pub unsafe fn erase_no_drop(&mut self, item: &Bucket<T>) {
    ...
    let empty_before = Group::load(self.ctrl(index_before))
      .match_empty();
    let empty_after = Group::load(self.ctrl(index))
      .match_empty();

    // If we are inside a continuous block of Group::WIDTH
    // full or deleted cells then a probe window may have seen
    // a full block when trying to insert. We therefore need to
    // keep that block non-empty so that lookups will
    // continue searching to the next probe window.
    //
    // Note that in this context `leading_zeros` refers to the
    // bytes at the end of a group, while `trailing_zeros`
    // refers to the bytes at the begining of a group.
  ...
  unsafe fn set_ctrl(&self, index: usize, ctrl: u8) {
    ...
    // because we mask the initial index for unaligned loads,
    // but we write it
    // anyways because it makes the set_ctrl implementation simpler.
    //
    // If there are fewer buckets than Group::WIDTH then this code
    // will replicate the buckets at the end of the trailing group.
    // For example with 2 buckets and a group size of 4, the control
    // bytes will look like this:
    //
    //     Real    |             Replicated
    // ---------------------------------------------
    // | [A] | [B] | [EMPTY] | [EMPTY] | [A] | [B] |
    ...
    for pos in self.probe_seq(hash) {
      unsafe {
        ...
        if let Some(bit) = group.match_empty_or_deleted().lowest_set_bit() {
          ...
          // slot (due to the load factor) before hitting the
          // trailing control bytes (containing EMPTY).
  ...
  /// Marks all table buckets as empty without dropping
  /// their contents.
  #[inline]
  ...
  #[inline]
  pub fn shrink_to(&mut self, min_size: usize,
    hasher: impl Fn(&T) -> u64)
  {
    // Calculate the minimal number of elements that we need to
    // reserve space for.
    ...
    // Calculate the number of buckets that we need for this number
    // of elements. If the calculation overflows then the requested
    // bucket count must be larger than what we have right and
      ...
      fallability: Fallibility) -> Result
    {
      ...
      if new_items <= full_capacity / 2 {
        // Rehash in-place without re-allocating if we have plenty
        // of spare capacity that is locked up due to DELETED
        // entries.
        self.rehash_in_place(hasher);
        Ok(())
      } else {
        // Otherwise, conservatively resize to at least the next
        // size up to avoid churning deletes into frequent rehashes.
        self.resize(
    ...
    ///
    /// If `hasher` panics then some the table's contents may be
    /// lost.
    fn rehash_in_place(&mut self, hasher: impl Fn(&T) -> u64) {
      unsafe {
        // Bulk convert all full control bytes to DELETED,
        // and all DELETED control bytes to EMPTY.
        // This effectively frees up all buckets containing a
        // DELETED entry.
        for i in (0..self.buckets()).step_by(Group::WIDTH) {
          ...
          // Fix up the trailing control bytes.
          // See the comments in set_ctrl for the handling
          // of tables smaller than the group width.
          ...
          'outer: for i in 0..guard.buckets() {
            ...
            'inner: loop {
              ...
              let probe_index = |pos: usize| {
                (pos.wrapping_sub(guard.probe_seq(hash).pos) & guard.bucket_mask) / Group::WIDTH
              };
              ...
              // We are moving the current item to a new position.
              // Write our H2 to the control byte of the new
              // position.
  ...
  #[inline]
  pub fn find(&self, hash: u64, mut eq: impl FnMut(&T) -> bool)
    -> Option<Bucket<T>>
  {
    unsafe {
      for pos in self.probe_seq(hash) {
  ...
  /// the caller to ensure that the `Table` outlives the `RawIter`.
  ...
  /// freeing the memory. It is up to the caller to ensure that the `Table`
